...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    ...
    private int selectPosition = -1;//当前显示页索引
    ...
    //设置图片url
    public static View setImageUrl(String url, Activity activity) {
        View view = activity.getLayoutInflater().inflate(R.layout.cylcle_view_imageview, null);
        SimpleDraweeView simpleDraweeView = (SimpleDraweeView) view.findViewById(R.id.sdv_carousel_avatar);
        simpleDraweeView.setImageURI(url);
        return view;
    }

    public void setData(List<View> views, List<ADInfo> list, ImageCycleViewListener listener, Context context) {
        setData(views, list, listener, 0, context);
    }
     ...
     */
    public void setData(List<View> views, List<ADInfo> list, ImageCycleViewListener listener, int showPosition, Context context) {
     ...
     * 指示器是否展示
     *
     * @param flag true 展示  false  不展示  默认展示
     */
    public void indicatorLayoutHideOrShow(boolean flag) {
        if (flag) {
        ...
        if (isCycle) {
            ...
            if (mImageCycleViewListener != null) {
                if (selectPosition != position) {
                    ...
                    mImageCycleViewListener.onChangeImage(infos.get(selectPosition), position);
                }
            }
        }
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setBackgroundResource(R.mipmap.carousel_default);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition].setBackgroundResource(R.mipmap.carousel_selected);
    }
    ...
    public static interface ImageCycleViewListener {
         ...
         * @param position
         */
        void onChangeImage(ADInfo info, int position);
    }
}